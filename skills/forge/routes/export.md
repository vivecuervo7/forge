...
Snippet **comments are dropped** from the inlined copy — all of them. A snippet's commentary is written for the library (who authored it, how it's been patched, why a selector was chosen) and in a shipped artifact it buries the code: on a real export it was 44% of the file. Two exceptions: comments that *do* something (`@ts-expect-error`, `eslint-disable`, `prettier-ignore` and friends) are preserved, since removing a suppression re-arms the rule it silenced; and text inside strings or template literals is untouched, so a `// …` line in script passed to `addInitScript`/`evaluate` survives as the source it is.

The spec's own comments are never touched. They're the author's documentation of the scenario — on a repro spec, the root-cause write-up is the most valuable thing in the file.
