...
node <PLUGIN_ROOT>/scripts/forge-cli.mjs run-spec \
  --spec <FORGE_ROOT>/specs/<resolved-name>.spec.ts --dashboard
# If RECORD_AS is set, append: --record-as <RECORD_AS>
```

Run **`--dashboard`** — `/forge run` is the watch-it / evidence path, and the flag renders the headless run live in the Playwright dashboard (the same window drives render in), without a browser window stealing focus. Swap to `--headed` only when the user explicitly asks for a real window; append `--slow-mo <ms>` when they want a fast replay paced for watching. For a headless CI check the user runs the spec through their own runner (`npx playwright test`) directly.
...
- **No team involved.** Thin script invocation. Don't spawn the driver or curator teammates. Re-authoring is `/forge spec`'s job.
