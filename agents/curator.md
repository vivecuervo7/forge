...
- **Drove fresh, but a too-broad snippet partially covers it** (the driver drove around an existing snippet because it did too much) → **split** the broad snippet into composable pieces so next time the right-sized one exists. (This helps the *next* drive, not the current one — that's expected.) When you split or refactor, carry each piece's caveats forward verbatim — never upgrade a fragility note (e.g. `FRAGILE SELECTOR — re-inspect if…`) into a "confirmed working" claim for a path this drive didn't actually exercise.
...
**Keep the form and the timeout as written.** `expect(locator).toBeVisible({ timeout })` is bounded by that timeout; rewriting it as `locator.waitFor({ state: 'visible' })` is *not* equivalent, because the project config leaves `actionTimeout` unset, so an unmet `waitFor` pends to the entire test timeout and surfaces as a mysterious "Test timeout exceeded" instead of a failed wait. That rewrite has happened, and it converts a safe gate into the exact footgun `lint-spec` exists to catch. The timeout itself is derived from how long the action actually took to settle, so it already fits this flow.
