...
| File | Purpose |
|---|---|
| [`forge/hints/forge.md`](./forge/hints/forge.md) | The single operate-the-app hint. Opens with the auth + multi-account contract — a three-row account table maps account keywords (`customer`, `customer2`, `admin`) to env-key pairs — then continues into a doc-grounded selector inventory + known-gotchas list, sourced from the app's [official docs](https://testsmith-io.github.io/practice-software-testing/). Includes the Angular zone.js `dispatchEvent` quirk, the search-box debounce, the two-click payment flow — project-specific knowledge that turns iterate-to-discover into first-try-pass. **The shape to copy when documenting your project's accounts, structure, selectors, and gotchas.** |
...
**What to look for:**
- The driver invokes the three seeded snippets in sequence; the curator authors nothing new.
...
Now the task includes a step the seeded library doesn't cover (login). The driver invokes the three seeded snippets for the cart steps, and the curator authors a new snippet for the login step.
...
**What this demonstrates:** the library grows compositionally. The driver and curator treat the existing library as the source of truth for what's already covered and only author what's genuinely new.
...
Same two agents as before — spec mode doesn't add anyone. The driver runs the full checkout, then composes a self-contained `.spec.ts` from its own drive trace and verifies it cold from a fresh browser context; the curator authors the new checkout-step snippets (billing address, payment, confirmation) as they're driven.
...
- The driver runs the spec cold and self-fixes. It may pass first-try, or iterate — that's normal. When a snippet's live-drive behaviour diverges from cold-start (timing, ordering, hidden form state), the driver routes a patch-request to the curator, which fixes the snippet, and the spec re-runs. It converges or escalates to you.
...
Teach mode is a collaborative drive: forge drives a step at a time while you walk it through, and the gotchas you flag get baked into the snippet body. Same two agents as any drive — the curator just has you in the loop, shaping what it captures.
...
1. **Author your `forge/hints/forge.md`.** It's the single operate-the-app file. Open with the account-table structure and adapt the rows to your project's test accounts — keep the env-key column, it's the bridge between user-facing keywords and `process.env` names. Then continue into the drive-the-app sections: app overview, routes table, selector inventory grouped by element class, login flow, known gotchas. The doc-grounded approach (read your app's docs, transcribe into `forge.md`) is the highest-leverage hint-authoring investment.

2. **Copy `.env.example` to `.env`** (or use direnv / dotenv-cli / your shell). Forge takes no opinion on how values reach `process.env`, only that they're there when the driver runs.

3. **Run forge against your target.** Drive a few tasks. Watch the library accrete. Try spec mode on the flow that matters most for CI. Try teach mode for the flows with quirks the agent shouldn't be expected to discover.
