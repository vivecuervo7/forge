...
- A `browser-session` skill — Claude knows how to ensure the `forge` session exists, invoke a registered snippet (possibly with arg overrides), and delegate authoring to an agent when no snippet covers the request.
